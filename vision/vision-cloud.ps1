...
# 前置：在 open.bigmodel.cn 注册获取 API Key，写入环境变量 VISION_API_KEY（或 -ApiKey 参数传入）
...
# 取密钥：优先 -ApiKey，其次环境变量 VISION_API_KEY，最后同目录/.env 或上级 .env
if (-not $ApiKey) { $ApiKey = $env:VISION_API_KEY }
if (-not $ApiKey) {
  foreach ($c in @((Join-Path $PSScriptRoot '.env'), (Join-Path (Split-Path $PSScriptRoot -Parent) '.env'))) {
    if (Test-Path $c) {
      $line = Select-String -Path $c -Pattern '^VISION_API_KEY=' -ErrorAction SilentlyContinue | Select-Object -First 1
      if ($line) { $ApiKey = $line.Line.Substring($line.Line.IndexOf('=') + 1).Trim().Trim('"'); break }
    }
  }
}
if (-not $ApiKey) {
  Write-Output "ERR: 未配置 API Key。请到 https://open.bigmodel.cn 注册获取（GLM-4V-Flash 免费），"
  Write-Output "然后设置环境变量 VISION_API_KEY（或运行: 本脚本 -ApiKey sk-xxx）"
  exit 1
}
