...
给 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 智能体（Agent）用的四个实战技能包：
**ComfyUI 图像生成 / 视觉眼睛 / 浏览器自动化 / 交接维护**。

> 这些技能来自一台真实运行 dsh 的 Windows 机器上的日常实践——每个脚本都经过实际任务验证，
> 踩坑记录直接写在对应 SKILL.md 里。适合想让自己的 Agent "会出图、会看图、会上网、会交接"的用户。
