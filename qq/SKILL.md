...
> 用途：给 AI「QQ 出口」——通过本机 NapCat（小号 <QQ_ID_ALT>「政务大臣」在线挂机）向宿主 QQ 号发送私聊消息、查询政务大臣在线状态与好友列表、**接收宿主 QQ 发来的图片**
...
| 脚本 | 能力 | 用法 |
|---|---|---|
| `qq-send.ps1` | 给指定 QQ 号发私聊消息 | `-UserId <QQ_ID_MAIN> -Message "你好"` |
| `qq-info.ps1` | 查询政务大臣登录状态 / 好友列表 | `-Friends`（列好友） |

## 调用示例

```powershell
# 给宿主发一条 QQ 消息
powershell -File qq-send.ps1 -UserId <QQ_ID_MAIN> -Message "宿主，政务大臣在 Chatbox 给您捎话：任务已完成"
...
- **机制**：宿主（白名单大号）给政务大臣小号发图片 → NapCat 上报 → qq-bridge 自动下载保存到
  `{DOWNLOADS}\QQ收图\`，文件名 `QQ-<YYYYMMDDHHmmss>-<QQ号>.<ext>`；元信息写 `latest.json`
  （时间/发送者/文件名/路径），每次收图覆盖
- **带斜杠指令的图片**：先收图，再照常执行指令（如 `/政务大臣 看看这张图`）
- **AI 侧查看流程**（宿主在 Chatbox 说「看QQ收的图」时）：
  1. 读 `{DOWNLOADS}\QQ收图\latest.json` 拿最新图片路径（没有则列出目录最新 5 张让宿主选）
...
| QQ | 昵称 | 说明 |
|---|---|---|
| <QQ_ID_MAIN> | 比方 | 宿主大号（白名单） |
| 2374387456 | — | 宿主大号（白名单） |

> 发消息前确认目标号在政务大臣好友列表（`qq-info.ps1 -Friends`），非好友私聊会失败。
...
1. **前置检查**：调用前先 `qq-info.ps1` 确认政务大臣在线（NapCat 3000 端口通）；NapCat 重启后 QQ 需重新扫码登录
2. **消息内容**：纯文本；含特殊字符时用单引号包裹；消息长度建议 ≤1500 字（超出 QQ 限制会失败）
3. **隐私**：QQ 消息会经过腾讯服务器，敏感内容（密钥/密码）严禁通过 QQ 发送
4. **只发不读**：本技能只负责「发消息」「查状态」「收宿主的图」，不读取 QQ 聊天记录（qq-bridge 侧的白名单与斜杠指令体系不变）
